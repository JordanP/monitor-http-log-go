...
func (m *mockTSDB) GetAvgRate(_ time.Duration) (float64, float64) {
	...
	if m.i < len(m.avgHitRates) {
		return m.avgHitRates[m.i], 0
	}
	// When we've ran out of values, always return the last value
	return m.avgHitRates[len(m.avgHitRates)-1], 0
}

func TestAlerterWhenAvgBelowThreshold(t *testing.T) {
	threshold := 42.0
	tsdb := &mockTSDB{avgHitRates: []float64{threshold - 1}}
...
func TestAlerterStateTransitions(t *testing.T) {
	threshold := 42.0
	tsdb := &mockTSDB{avgHitRates: []float64{threshold + 1, threshold - 1}}
